...
import { rand, delRedisKeys } from '../../model/util';
import {
    IfAtSpot,
    GetBackpackItem,
    SetAction, IsNew, RegNew,
    SetBackpack, AddItemToBackpack, AddSpiritStoneToBackpack,
    SetBattle, GetNewBattle,
    SetEquipment,
    SetLevel,
    SetLife,
    SetSkill,
    SetTalent, GetNewTalent,
    SetWarehouse,
    AddUid,
} from '../../model/Cache';
...
export default class start extends plugin {
    ...
    NewSupport = async (e) => {
        ...
        const weapon = await GetBackpackItem('烂铁匕首');
        const wealth = 100;
        e.reply(`[修仙联盟]方正\n看你骨骼惊奇，就送你一把[${weapon.name}]吧，还有这里有${wealth}灵石，已经放到你的储物袋里了，可以#储物袋查看.`);
        AddItemToBackpack(e.user_id, weapon, 1);
        AddSpiritStoneToBackpack(e.user_id, wealth);
        RegNew(e.user_id);
    }
